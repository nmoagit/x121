//! Avatar Settings Dashboard logic (PRD-108).
//!
//! Provides types, validation functions, JSON-merge helpers, and label
//! formatters for the unified avatar dashboard.  The `core` crate
...
/// Logical sections of the avatar dashboard.
...
pub fn build_action_url(avatar_id: i64, category: &MissingItemCategory) -> String {
    match category {
        ...
        MissingItemCategory::ApprovedVariant => {
            format!("/avatars/{avatar_id}/image-variants")
        }
        MissingItemCategory::MetadataComplete => {
            format!("/avatars/{avatar_id}/metadata")
        }
        MissingItemCategory::PipelineSetting => {
            format!("/avatars/{avatar_id}/settings")
        }
    }
}
