//! Avatar library validation and field sync utilities (PRD-60).
//!
//! Provides constants, validation helpers, and field-level synchronisation
//! classification for the cross-project avatar library feature.
...
/// Maximum number of linked fields allowed on a single project-avatar link.
...
/// Per-field synchronisation status between a library avatar and a project copy.
...
pub struct FieldSyncStatus {
    ...
    pub status: String,
    /// The value in the library avatar's master_metadata (if present).
    pub library_value: Option<serde_json::Value>,
    /// The value in the project avatar's metadata (if present).
