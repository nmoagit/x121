//! Folder scanner and image classifier for the character ingest pipeline (PRD-113).
//!
//! Scans a root directory for character folders, detects images and metadata
...
/// A scanned character folder with its detected contents.
#[derive(Debug, Clone, Serialize)]
pub struct ScannedCharacterFolder {
    /// The original folder name.
    pub folder_name: String,
    /// The parsed character name with confidence.
...
/// Scan a root directory for character folders.
///
/// Each subdirectory of `root` is treated as a character folder. Files within
/// each folder are classified as images, metadata, or other files.
pub async fn scan_character_folders(
    root: &Path,
) -> Result<Vec<ScannedCharacterFolder>, std::io::Error> {
    ...
    while let Some(entry) = entries.next_entry().await? {
        ...
        let parsed_name = name_parser::parse_character_name(&folder_name);
        ...
        results.push(ScannedCharacterFolder {
            folder_name,
            parsed_name,
            images,
            metadata_file,
...
/// Scan the contents of a single character folder.
///
