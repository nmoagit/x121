//! Avatar metadata version entity model and DTOs.

use serde::{Deserialize, Serialize};
use sqlx::FromRow;
use x121_core::types::{DbId, Timestamp};

/// A row from the `avatar_metadata_versions` table.
#[derive(Debug, Clone, FromRow, Serialize)]
pub struct AvatarMetadataVersion {
    pub id: DbId,
    pub avatar_id: DbId,
    pub version_number: i32,
    pub metadata: serde_json::Value,
    pub source: String,
    pub source_bio: Option<serde_json::Value>,
    pub source_tov: Option<serde_json::Value>,
    pub generation_report: Option<serde_json::Value>,
    pub is_active: bool,
    pub notes: Option<String>,
    pub rejection_reason: Option<String>,
    pub outdated_at: Option<Timestamp>,
    pub outdated_reason: Option<String>,
    pub approval_status: String,
    pub approved_by: Option<DbId>,
    pub approved_at: Option<Timestamp>,
    pub approval_comment: Option<String>,
    pub deleted_at: Option<Timestamp>,
    pub created_at: Timestamp,
    pub updated_at: Timestamp,
}

/// DTO for creating a new avatar metadata version.
#[derive(Debug, Clone, Deserialize)]
pub struct CreateAvatarMetadataVersion {
    pub avatar_id: DbId,
    pub metadata: serde_json::Value,
    pub source: String,
    pub source_bio: Option<serde_json::Value>,
    pub source_tov: Option<serde_json::Value>,
    pub generation_report: Option<serde_json::Value>,
    pub is_active: Option<bool>,
    pub notes: Option<String>,
}

/// DTO for updating a avatar metadata version.
#[derive(Debug, Clone, Deserialize)]
pub struct UpdateAvatarMetadataVersion {
    pub notes: Option<String>,
    pub rejection_reason: Option<String>,
}

/// Request body for rejecting a metadata version's reviewer approval.
#[derive(Debug, Clone, Deserialize)]
pub struct RejectMetadataApprovalRequest {
    pub comment: Option<String>,
}
