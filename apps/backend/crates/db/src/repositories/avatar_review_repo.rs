//! Repository for avatar review assignment, decision, and audit tables (PRD-129).
//!
...
/// Column list for `avatar_review_assignments` queries.
const ASSIGNMENT_COLUMNS: &str = "id, avatar_id, reviewer_user_id, assigned_by, \
    reassigned_from, review_round, status, started_at, completed_at, deadline, \
    created_at, updated_at";

/// Column list for `avatar_review_decisions` queries.
const DECISION_COLUMNS: &str = "id, assignment_id, avatar_id, reviewer_user_id, \
    decision, comment, review_round, review_duration_sec, decided_at, created_at";

/// Column list for `avatar_review_audit_log` queries.
const AUDIT_COLUMNS: &str = "id, avatar_id, action, actor_user_id, target_user_id, \
...
/// Provides data access for avatar review assignments, decisions, and audit log.
pub struct AvatarReviewRepo;

impl AvatarReviewRepo {
    // -----------------------------------------------------------------------
    // Assignment CRUD
    // -----------------------------------------------------------------------

    /// Create a new review assignment for a avatar.
    ...
    ) -> Result<AvatarReviewAssignment, sqlx::Error> {
        let query = format!(
            "INSERT INTO avatar_review_assignments
                (avatar_id, reviewer_user_id, assigned_by, review_round, deadline)
             VALUES ($1, $2, $3, $4, $5)
             RETURNING {ASSIGNMENT_COLUMNS}"
        );
        sqlx::query_as::<_, AvatarReviewAssignment>(&query)
            .bind(avatar_id)
    ...
    ///
    /// Used when reassigning a avatar to a different reviewer.
    pub async fn create_assignment_with_reassign(
        ...
        let query = format!(
            "INSERT INTO avatar_review_assignments
                (avatar_id, reviewer_user_id, assigned_by, review_round, deadline, reassigned_from)
             VALUES ($1, $2, $3, $4, $5, $6)
             RETURNING {ASSIGNMENT_COLUMNS}"
        );
        sqlx::query_as::<_, AvatarReviewAssignment>(&query)
    ...
    /// Find the active assignment for a avatar, if any.
    pub async fn find_active_by_avatar(
        pool: &PgPool,
        avatar_id: DbId,
    ) -> Result<Option<AvatarReviewAssignment>, sqlx::Error> {
        let query = format!(
            "SELECT {ASSIGNMENT_COLUMNS} FROM avatar_review_assignments
    ...
    pub async fn find_by_id(
        ...
        let query =
            format!("SELECT {ASSIGNMENT_COLUMNS} FROM avatar_review_assignments WHERE id = $1");
        sqlx::query_as::<_, AvatarReviewAssignment>(&query)
            .bind(id)
            .fetch_optional(pool)
            .await
    ...
    pub async fn list_by_reviewer(
        ...
        sqlx::query_as::<_, ReviewQueueAvatar>(
            "SELECT
                ...
                (SELECT COUNT(*) FROM scenes s
                 WHERE s.avatar_id = c.id AND s.deleted_at IS NULL) AS scene_count,
                cra.created_at AS assigned_at,
                cra.deadline,
                cra.status
             FROM avatar_review_assignments cra
             JOIN avatars c ON c.id = cra.avatar_id
    ...
    pub async fn list_by_project(
        ...
        limit: i64,
        offset: i64,
    ) -> Result<Vec<AvatarReviewAssignment>, sqlx::Error> {
        ...
        sqlx::query_as::<_, AvatarReviewAssignment>(&sql)
            .bind(project_id)
            .bind(limit)
            .bind(offset)
            .fetch_all(pool)
    ...
    pub async fn start_review(
        ...
        let query = format!(
            "UPDATE avatar_review_assignments
             SET started_at = NOW()
             WHERE id = $1
             RETURNING {ASSIGNMENT_COLUMNS}"
        );
    ...
    pub async fn count_active_by_reviewer(
        ...
        let row = sqlx::query_as::<_, CountRow>(
            "SELECT COUNT(*)::BIGINT AS count
             FROM avatar_review_assignments
        ...
        sqlx::query_as::<_, ReviewerWorkload>(
            "SELECT
                ...
                COUNT(*) FILTER (WHERE cra.status = 'completed' AND EXISTS (
                    SELECT 1 FROM avatar_review_decisions d
             ...
             LEFT JOIN avatar_review_assignments cra ON cra.reviewer_user_id = u.id
             LEFT JOIN avatars c ON c.id = cra.avatar_id AND c.project_id = $1
    ...
    /// Find the last completed assignment for a avatar (for re-assignment to the same reviewer).
    pub async fn last_completed_for_avatar(
        pool: &PgPool,
        avatar_id: DbId,
    ) -> Result<Option<AvatarReviewAssignment>, sqlx::Error> {
        let query = format!(
    ...
    ) -> Result<AvatarReviewDecision, sqlx::Error> {
        let query = format!(
            "INSERT INTO avatar_review_decisions
                (assignment_id, avatar_id, reviewer_user_id, decision,
                 comment, review_round, review_duration_sec)
             VALUES ($1, $2, $3, $4, $5, $6, $7)
             RETURNING {DECISION_COLUMNS}"
        );
        sqlx::query_as::<_, AvatarReviewDecision>(&query)
            .bind(assignment_id)
            .bind(avatar_id)
            .bind(reviewer_user_id)
            .bind(decision)
    ...
    /// List all decisions for a avatar, most recent first.
    pub async fn list_decisions_by_avatar(
        pool: &PgPool,
        avatar_id: DbId,
    ) -> Result<Vec<AvatarReviewDecision>, sqlx::Error> {
        let query = format!(
            "SELECT {DECISION_COLUMNS} FROM avatar_review_decisions
             WHERE avatar_id = $1
    ...
    /// Record an audit log entry for a avatar review action.
    ...
    ) -> Result<AvatarReviewAuditEntry, sqlx::Error> {
        let query = format!(
            "INSERT INTO avatar_review_audit_log
                (avatar_id, action, actor_user_id, target_user_id, comment, metadata)
             VALUES ($1, $2, $3, $4, $5, $6)
             RETURNING {AUDIT_COLUMNS}"
        );
        sqlx::query_as::<_, AvatarReviewAuditEntry>(&query)
            .bind(avatar_id)
            .bind(action)
            .bind(actor_user_id)
            .bind(target_user_id)
    ...
    /// List audit log entries for a avatar, most recent first.
    pub async fn list_audit_by_avatar(
        pool: &PgPool,
        avatar_id: DbId,
        limit: i64,
        offset: i64,
    ) -> Result<Vec<AvatarReviewAuditEntry>, sqlx::Error> {
        let query = format!(
            "SELECT {AUDIT_COLUMNS} FROM avatar_review_audit_log
             WHERE avatar_id = $1
             ORDER BY created_at DESC
             LIMIT $2 OFFSET $3"
        );
    ...
    ///
    /// Joins through `avatars` to filter by `project_id`, and supports
    ...
    pub async fn list_audit_by_project(
        ...
        let sql = format!(
            "SELECT {prefixed_cols}
             FROM avatar_review_audit_log a
             JOIN avatars c ON c.id = a.avatar_id
             WHERE {where_clause}
             ORDER BY a.created_at DESC
             LIMIT $2 OFFSET $3"
        );

        let mut q = sqlx::query_as::<_, AvatarReviewAuditEntry>(&sql)
            .bind(project_id)
            .bind(limit)
            .bind(offset);
    ...
    pub async fn export_audit_by_project(
        ...
        let mut q = sqlx::query_as::<_, AvatarReviewAuditEntry>(&sql).bind(project_id);
    ...
    /// Update the review status of a avatar.
    pub async fn update_review_status(
        pool: &PgPool,
        avatar_id: DbId,
        review_status_id: i16,
    ) -> Result<(), sqlx::Error> {
        sqlx::query("UPDATE avatars SET review_status_id = $2 WHERE id = $1")
            .bind(avatar_id)
    ...
    /// List unassigned avatars for a project (review_status_id IN (1=unassigned, 7=re_queued)).
