...
use trulience_db::models::image::{CreateSourceImage, SourceImage, UpdateSourceImage};
use trulience_db::repositories::SourceImageRepo;
...
pub async fn delete(
    ...
    let deleted = SourceImageRepo::delete(&state.pool, id).await?;
    if deleted {
