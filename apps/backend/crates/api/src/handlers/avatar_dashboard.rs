//! Handlers for the avatar settings dashboard (PRD-108).
//!
//! Provides a unified dashboard endpoint that aggregates avatar data
//! from multiple tables, and a PATCH endpoint for partially updating
//! avatar settings JSONB.

use axum::extract::{Path, State};
use axum::response::IntoResponse;
use axum::Json;

use x121_core::avatar_dashboard::validate_settings_update;
...
/// Aggregated dashboard data returned by `GET /avatars/{id}/dashboard`.
#[derive(Debug, serde::Serialize)]
pub struct AvatarDashboardData {
    pub avatar_id: DbId,
...
/// An enabled scene_type+track assignment for this avatar.
...
/// GET /avatars/{id}/dashboard
///
/// Returns aggregated dashboard data for a single avatar including:
/// - Avatar info (name, project)
/// - Source image count
...
pub async fn get_dashboard(
    ...
    let avatar = sqlx::query_as::<_, AvatarRow>(
        "SELECT id, name, project_id, group_id, settings FROM avatars WHERE id = $1 AND deleted_at IS NULL",
    )
    .bind(avatar_id)
    ...
    // LEFT JOINed to actual scenes and video versions for this avatar.
    // $1 = avatar_id, $2 = project_id, $3 = group_id (nullable)
    let scene_assignments = sqlx::query_as::<_, SceneAssignment>(
        ...
        LEFT JOIN scenes sc
            ON sc.scene_type_id = ec.scene_type_id AND sc.track_id = ec.track_id
               AND sc.avatar_id = $1 AND sc.deleted_at IS NULL
    ...
    .bind(avatar_id)
    .bind(avatar.project_id)
    .bind(avatar.group_id)
    .fetch_all(&state.pool)
    .await?;

    // Video version summary across all scene+track combos for this avatar.
    ...
    let segment_counts_row = sqlx::query_as::<_, SegmentCountsRow>(
        "SELECT
         ...
         WHERE sc.avatar_id = $1 AND sc.deleted_at IS NULL AND svv.deleted_at IS NULL",
    )
    .bind(avatar_id)
    .fetch_one(&state.pool)
    .await?;

    let dashboard = AvatarDashboardData {
        avatar_id: avatar.id,
        ...
        settings: avatar.settings.unwrap_or(serde_json::json!({})),
        readiness,
...
/// PATCH /avatars/{id}/settings
///
/// Partially update a avatar's settings JSONB using PostgreSQL `||`
/// operator (merge, not replace). After update, invalidate the readiness
/// cache for the avatar.
pub async fn patch_settings(
    ...
    let _ = ReadinessCacheRepo::delete_by_avatar_id(&state.pool, avatar_id).await;

    tracing::info!(
        user_id = auth.user_id,
        avatar_id = avatar_id,
        "Avatar settings patched"
    );
