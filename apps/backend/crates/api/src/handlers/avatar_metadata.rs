//! Handlers for avatar metadata editing (PRD-66).
//!
//! Provides endpoints for reading and writing avatar metadata fields,
//! completeness calculation, and CSV export/import with diff preview.
//!
//! Metadata is stored in the `avatars.metadata` JSONB column.
...
use x121_db::models::avatar::Avatar;
...
use crate::handlers::avatar_metadata_version::build_manual_version_input;
...
pub struct AvatarMetadataResponse {
    pub avatar_id: DbId,
...
/// Extract the metadata map from a avatar, defaulting to empty object.
fn avatar_metadata_map(avatar: &Avatar) -> serde_json::Map<String, serde_json::Value> {
    avatar
        .metadata
        .as_ref()
...
///
/// NOTE: The frontend `groupFieldsIntoSections()` in `avatars/types.ts`
...
/// Build a structured response from a avatar and field definitions.
///
...
///
/// Return structured metadata for a single avatar, including field
/// definitions, current values, and completeness status.
pub async fn get_avatar_metadata(
    ...
    let fields = load_template_fields(&state.pool, Some(avatar.project_id)).await?;
    let response = build_metadata_response(&avatar, &fields);

    Ok(Json(DataResponse { data: response }))
}

/// GET /api/v1/avatars/{avatar_id}/metadata/template
///
/// Return the active metadata template and its fields for a avatar.
...
/// PUT /api/v1/avatars/{avatar_id}/metadata
///
...
pub async fn update_avatar_metadata(
    State(state): State<AppState>,
    Path(avatar_id): Path<DbId>,
    ...
    let mut existing = avatar_metadata_map(&avatar);
    ...
    let updated = AvatarRepo::update(
        &state.pool,
        avatar_id,
        &x121_db::models::avatar::UpdateAvatar {
            name: None,
            status_id: None,
    ...
    // Source file uploads (_source_bio, _source_tov) are stored on the avatar
    ...
    let should_create_version = !only_source_keys
        && !all_fields_cleared
        && match AvatarMetadataVersionRepo::find_active(&state.pool, avatar_id).await {
    ...
    if should_create_version {
        ...
        let _ = AvatarMetadataVersionRepo::create_as_active(&state.pool, &version_input).await;
    }
...
/// GET /api/v1/projects/{project_id}/avatars/metadata
///
/// Return metadata for all avatars in a project (for spreadsheet view).
pub async fn list_project_metadata(
    ...
    let responses: Vec<AvatarMetadataResponse> = avatars
        .iter()
...
/// GET /api/v1/avatars/{avatar_id}/metadata/completeness
///
/// Return completeness status for a single avatar.
pub async fn get_completeness(
    ...
    let fields = load_template_fields(&state.pool, Some(avatar.project_id)).await?;
    let metadata = avatar_metadata_map(&avatar);
    let result = calculate_completeness(avatar.id, &metadata, &fields);

    Ok(Json(DataResponse { data: result }))
}

/// GET /api/v1/projects/{project_id}/avatars/metadata/completeness
///
...
pub async fn get_project_completeness(
    ...
    let avatar_data: Vec<(i64, serde_json::Map<String, serde_json::Value>)> = avatars
        .iter()
        .map(|c| (c.id, avatar_metadata_map(c)))
        .collect();

    let result = calculate_project_completeness(&avatar_data, &fields);

    Ok(Json(DataResponse { data: result }))
}

/// GET /api/v1/projects/{project_id}/avatars/metadata/csv
///
/// Export all avatar metadata as CSV.
pub async fn export_metadata_csv(
    ...
    let avatar_data: Vec<(i64, String, serde_json::Map<String, serde_json::Value>)> = avatars
        .iter()
        .map(|c| (c.id, c.name.clone(), avatar_metadata_map(c)))
        .collect();

    let csv = build_csv(&avatar_data, &fields);
...
pub async fn import_metadata_csv_preview(
    ...
    // Build lookup by avatar ID.
    let char_by_id: std::collections::HashMap<DbId, &Avatar> =
        avatars.iter().map(|c| (c.id, c)).collect();
    ...
    for (row_idx, record) in records.iter().enumerate() {
        ...
        let Some(avatar) = char_by_id.get(&char_id) else {
            unmatched += 1;
            continue;
        };
        ...
        if !errors.is_empty() {
            validation_errors.push(CsvRecordError {
                row_index: row_idx,
                avatar_id: Some(char_id),
                errors,
            });
        }

        // Compute diffs.
        let existing = avatar_metadata_map(avatar);
