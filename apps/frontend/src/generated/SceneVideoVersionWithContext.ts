 ...
 */
export type SceneVideoVersionWithContext = { avatar_id: number, avatar_name: string, avatar_is_enabled: boolean, scene_type_name: string, track_name: string, project_id: number, project_name: string, 
/**
 * Parent version's `version_number`, for derived clips.
 * `None` when this row is not a derived clip.
 */
parent_version_number: number | null, id: number, scene_id: number, version_number: number, source: string, file_path: string, file_size_bytes: number | null, duration_secs: number | null, width: number | null, height: number | null, frame_rate: number | null, preview_path: string | null, 
/**
