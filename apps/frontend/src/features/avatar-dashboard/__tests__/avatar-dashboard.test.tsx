...
import { avatarDashboardKeys } from "../hooks/use-avatar-dashboard";
import type {
  GenerationSummary,
  MissingItem,
  SceneAssignment,
...
const makeMissingItem = (
  ...
  label: "source image",
  actionUrl: "/avatars/1/source-images",
  ...overrides,
});
...
describe("avatarDashboardKeys", () => {
  it("all key is stable", () => {
    expect(avatarDashboardKeys.all).toEqual(["avatar-dashboard"]);
  });

  it("dashboard key includes avatarId", () => {
    expect(avatarDashboardKeys.dashboard(42)).toEqual([
      "avatar-dashboard",
      "dashboard",
      42,
    ]);
  ...
  it("settings key includes avatarId", () => {
    expect(avatarDashboardKeys.settings(7)).toEqual([
      "avatar-dashboard",
      "settings",
      7,
    ]);
