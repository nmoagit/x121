...
export function AvatarReviewControls({
  avatarId,
  reviewStatusId,
  assignmentId,
  assignedReviewerUserId,
}: AvatarReviewControlsProps) {
  const user = useAuthStore((s) => s.user);
  const startReview = useStartReview();
  const submitDecision = useSubmitDecision();
  const submitRereview = useSubmitForRereview();
  const { data: history } = useAvatarReviewHistory(avatarId);
        ...
        <div className="flex items-center gap-2">
          ...
          {status === "rework" && (
            <Button
              variant="primary"
              size="xs"
              onClick={() => submitRereview.mutate(avatarId)}
