  ...
  return (
    ...
    >
      <div className="relative h-10 w-16 shrink-0 rounded overflow-hidden bg-[var(--color-surface-secondary)]">
        {/* PRD-169: only mount <video> when transcode is completed. */}
        {!clip.file_purged && clip.transcode_state === "completed" && (
          <video
            src={videoSrc}
      ...
      <div className="flex-1 min-w-0">
        <div className={`flex items-center gap-2 ${TYPO_DATA}`}>
          {clip.clip_index != null && (
            <span className="text-[var(--color-data-cyan)] font-semibold">#{clip.clip_index}</span>
          )}
        ...
        <div className="flex items-center gap-2 font-mono text-[10px] text-[var(--color-text-muted)] mt-0.5">
          ...
          {clip.qa_status !== "pending" && (
            <>
              ...
              <span className={TERMINAL_STATUS_COLORS[clip.qa_status] ?? ""}>{clip.qa_status}</span>
            </>
          )}
  ...
  return (
    <Stack gap={4}>
      ...
      <ClipPlaybackModal
        ...
        onNext={
          playingIndex >= 0 && playingIndex < flatClips.length - 1
            ? () => setPlayingClip(flatClips[playingIndex + 1]!)
            : undefined
        }
        pipelineId={pipelineCtx?.pipelineId}
        meta={
          playingClip
            ? {
                projectName: "",
                avatarName: "",
                sceneTypeName: playingClip.scene_type_name,
                trackName: playingClip.track_name,
              }
            : undefined
        }
      />
