/**
 * Avatar overview tab showing identity, stats, and completeness (PRD-112).
 */
...
import { useBulkApprove } from "../hooks/use-avatar-detail";
import type { BulkApproveResult } from "../hooks/use-avatar-detail";
...
interface AvatarOverviewTabProps {
  avatar: Avatar;
  avatarId: number;
  projectId: number;
  /** Resolved blocking deliverables for this avatar. */
...
export function AvatarOverviewTab({
  avatar,
  avatarId,
  projectId,
  ...
  const metadataFieldCount = Object.keys(avatar.metadata ?? {}).length;
  ...
  return (
    <Stack gap={4}>
      ...
      {/* VoiceID status */}
      {(() => {
        const voiceId = getVoiceId(avatar.settings as Record<string, unknown> | null);
      ...
      >
        <p>
          ...
          <strong>{avatar.name}</strong> in the following sections:{" "}
