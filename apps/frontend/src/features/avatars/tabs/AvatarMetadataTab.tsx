/**
 * Avatar metadata tab — sectioned form driven by metadata template.
 *
 * Supports:
...
interface AvatarMetadataTabProps {
  avatarId: number;
  projectId: number;
}

export function AvatarMetadataTab({ avatarId, projectId }: AvatarMetadataTabProps) {
  const { data: metadataResponse, isLoading: metaLoading } = useAvatarMetadata(avatarId);
  const { data: templateResponse, isLoading: templateLoading } = useMetadataTemplate(avatarId);
  ...
  // Version hooks
  const { data: versions } = useMetadataVersions(avatarId);
  const generateMetadataApi = useGenerateMetadata(avatarId);
  const createVersion = useCreateManualVersion(avatarId);
  const activateVersion = useActivateVersion(avatarId);
  const rejectVersion = useRejectVersion(avatarId);
  const deleteVersion = useDeleteVersion(avatarId);
  const approveMetadata = useApproveMetadataVersion(avatarId);
  const unapproveMetadata = useUnapproveMetadataVersion(avatarId);
  const rejectMetadataApproval = useRejectMetadataApproval(avatarId);

  // Refinement hooks (PRD-125)
  const { data: refinementJobs } = useRefinementJobs(avatarId);
  const triggerRefinement = useTriggerRefinement(avatarId);
  const approveRefinement = useApproveRefinement(avatarId);
  const rejectRefinement = useRejectRefinement(avatarId);
  const clearOutdated = useClearOutdated(avatarId);
  const markOutdated = useMarkOutdated(avatarId);
  ...
  const handleGenerate = useCallback(() => {
    generateMetadataApi.mutate(
      ...
      {
        onSuccess: (data) => {
          // The backend created + activated a new version AND synced to avatars.metadata.
