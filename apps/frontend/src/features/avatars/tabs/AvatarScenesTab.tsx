/**
 * Avatar scenes tab — grid of scene cards with generation controls (PRD-112).
 *
 * Shows a card for every enabled scene_type × track combination (derived from
 * the three-level merge: catalogue → project → avatar override, cross-joined
...
import { useAvatarScenes, useCreateScene } from "@/features/scenes/hooks/useAvatarScenes";
...
interface AvatarScenesTabProps {
  avatarId: number;
  projectId: number;
  ...
  focusTrackId?: number;
  /** Whether the avatar is enabled. When false, all generation controls are disabled. */
...
export function AvatarScenesTab({ avatarId, projectId, focusSceneId, focusSceneTypeId, focusTrackId, avatarEnabled = true }: AvatarScenesTabProps) {
  ...
  const { data: scenes, isLoading: scenesLoading } = useAvatarScenes(avatarId, anyGenerating);
  ...
  const toggleSetting = useToggleAvatarSceneSetting(avatarId);
  ...
  const isDisabled = !avatarEnabled || importing || batchGenerate.isPending;
