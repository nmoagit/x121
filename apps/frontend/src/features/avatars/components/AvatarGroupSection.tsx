/**
 * Reusable expandable group section for avatar grids (PRD-135).
 *
 * Extracted from ProjectAvatarsTab so both the project tab and
 * Avatar Creator page can share the same group layout.
 */
...
/** Minimal avatar shape needed by the group section. */
export interface GroupSectionAvatar {
  id: number;
  name: string;
}
...
export interface AvatarGroupSectionProps<T extends GroupSectionAvatar> {
  /** HTML id for scroll-to-group anchoring. */
  sectionId?: string;
  /** Display name for the group header. */
  label: string;
  /** Avatars in this group (already filtered/sorted). */
  avatars: T[];
  ...
  /** Toggle selection of a single avatar. */
  onCharSelect: (charId: number) => void;
  /** Select or deselect a batch of avatar IDs. Pass [] to deselect all. */
  ...
  /** Render function for each avatar. Receives the avatar object. */
  renderCard: (avatar: T) => ReactNode;
  /** Optional subtitle shown next to the label (e.g. project name). */
  subtitle?: string;
  /** Message shown when the group has no avatars. */
  emptyMessage?: string;
}
...
export function AvatarGroupSection<T extends GroupSectionAvatar>({
  sectionId,
  label,
  avatars,
  expanded,
  ...
  const allSelected = avatars.length > 0 && charIds.every((id) => selectedCharIds.has(id));
