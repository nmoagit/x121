...
import { CharacterSceneOverrideEditor } from "../CharacterSceneOverrideEditor";
import type {
...
vi.mock("../hooks/use-prompt-management", () => ({
  ...
  useCharacterSceneOverrides: vi.fn(),
  useUpsertCharacterSceneOverrides: vi.fn(),
...
const OVERRIDE_WITH_FRAGMENTS: CharacterScenePromptOverride = {
  id: 200,
  character_id: 3,
  scene_type_id: 5,
  prompt_slot_id: 1,
...
function setupMocks({
  ...
  overrides?: CharacterScenePromptOverride[];
  ...
  vi.mocked(useCharacterSceneOverrides).mockReturnValue({
    data: overrides,
    isPending: overridesLoading,
    isError: false,
  } as ReturnType<typeof useCharacterSceneOverrides>);

  vi.mocked(useUpsertCharacterSceneOverrides).mockReturnValue({
    mutate: vi.fn(),
    isPending: false,
  } as unknown as ReturnType<typeof useUpsertCharacterSceneOverrides>);
...
describe("CharacterSceneOverrideEditor", () => {
  it("renders slot sections with base text", () => {
    ...
    renderWithProviders(
      <CharacterSceneOverrideEditor characterId={3} sceneTypeId={5} workflowId={10} />,
    );
