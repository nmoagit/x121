...
import type { BulkImportReport, CharacterSpeech, Language } from "../types";
...
export function CharacterSpeechTab({ characterId, projectId }: CharacterSpeechTabProps) {
  ...
  const parseEntries = useCallback((format: "json" | "csv", data: string): { entries: ParsedEntry[]; characters: string[] } => {
    ...
    if (format === "json") {
      ...
      if (lines.length >= 2) {
        ...
        characters.push(...slugs);
      }
    }
    return { entries, characters };
  }, []);

  /** Compare parsed entries against this character's existing speeches. */
  ...
  const handleSpeechDrop = useCallback((format: "json" | "csv", data: string) => {
    const { entries, characters } = parseEntries(format, data);
    const isMultiModel = characters.length > 1;
    // Only compare against existing speeches for single-character imports.
    // Multi-model imports go to many characters — per-entry comparison is misleading.
    const { existing, newEntries } = isMultiModel
      ? { existing: [] as ParsedEntry[], newEntries: entries }
      : analyzeEntries(entries);
    setDropAnalysis({ format, data, isMultiModel, characters, entries, existing, newEntries });
    setBulkResult(null);
      ...
      >
          ...
          <Stack gap={3}>
            ...
            <div className="flex items-center gap-3 font-mono text-xs">
              ...
              {dropAnalysis.isMultiModel && (
                <>
                  ...
                  <span><span className="text-cyan-400">{dropAnalysis.characters.length}</span> models</span>
                </>
              )}
            ...
            {dropAnalysis.isMultiModel && (
              <div>
                ...
                <div className="max-h-24 overflow-y-auto border border-[var(--color-border-default)] rounded-[var(--radius-sm)]">
                  {dropAnalysis.characters.map((slug) => (
            ...
            <div className="flex items-center justify-between pt-1 border-t border-[var(--color-border-default)]">
              <span className="font-mono text-xs text-[var(--color-text-muted)]">
                {dropAnalysis.isMultiModel
                  ? `${dropAnalysis.entries.length} entries across ${dropAnalysis.characters.length} models`
        ...
        {bulkResult && (
          <Stack gap={3}>
            ...
            {bulkResult.characters_matched.length > 0 && (
              <div>
                <p className="font-mono text-[10px] uppercase tracking-wide text-[var(--color-text-muted)] mb-1">
                  matched ({bulkResult.characters_matched.length})
                </p>
                <div className="max-h-32 overflow-y-auto border border-[var(--color-border-default)] rounded-[var(--radius-sm)]">
                  {bulkResult.characters_matched.map((name) => (
            ...
            {bulkResult.characters_unmatched.length > 0 && (
              <div>
                <p className="font-mono text-[10px] uppercase tracking-wide text-orange-400 mb-1">
                  unmatched ({bulkResult.characters_unmatched.length})
                </p>
                <div className="max-h-24 overflow-y-auto border border-[var(--color-border-default)] rounded-[var(--radius-sm)]">
                  {bulkResult.characters_unmatched.map((name) => (
