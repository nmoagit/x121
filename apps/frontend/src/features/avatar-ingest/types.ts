/**
 * TypeScript types for the avatar ingest pipeline (PRD-113).
 *
 * These types mirror the backend API response shapes from
 * `handlers/avatar_ingest.rs`, `handlers/metadata_template.rs`,
...
export interface AvatarIngestSession {
  id: number;
  project_id: number;
  status_id: number;
  source_type: string;
...
export interface AvatarIngestEntry {
  id: number;
  session_id: number;
...
export interface IngestSessionDetail {
  session: AvatarIngestSession;
  entries: AvatarIngestEntry[];
...
export type { MetadataTemplateField } from "@/features/avatars/types";
export type {
  MetadataTemplate,
