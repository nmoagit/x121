 ...
 * Supports text/CSV name input and displays parsed avatar entries
...
interface FolderImportWizardProps {
  projectId: number;
  onComplete: (avatarIds: number[]) => void;
  onCancel: () => void;
}
      ...
      <div className="p-6">
        ...
        <div className="mt-6">
          ...
          {currentStep === "input" && (
            <Stack gap={4}>
              ...
              <p className="text-sm text-muted-foreground">
                Enter one avatar name per line. Folder names, underscored
          ...
          {currentStep === "preview" && sessionData && (
            <Stack gap={4}>
              <h3 className="text-lg font-semibold">Review parsed avatars</h3>
              <p className="text-sm text-muted-foreground">
              ...
              <WizardNavFooter onBack={goBack} onCancel={handleCancel}>
                <Button onClick={handleValidate} disabled={validateSession.isPending}>
                  {validateSession.isPending ? <ContextLoader size={32} /> : "Validate"}
                </Button>
              </WizardNavFooter>
            </Stack>
          )}
              ...
              {confirmResult ? (
                ...
                <>
                  ...
                  <WizardNavFooter onBack={goBack} onCancel={handleCancel}>
                    ...
                    >
                      {confirmImport.isPending ? <ContextLoader size={32} /> : "Confirm Import"}
                    </Button>
                  </WizardNavFooter>
                </>
              )}
...
function ConfirmPreview({ entries }: { entries: AvatarIngestEntry[] }) {
  ...
  return (
    <div className="space-y-3 text-sm">
      <p>
        <strong>{included.length}</strong> avatars will be imported.
        {excluded.length > 0 && (
          <>
            {" "}
  ...
  return (
    <div className="space-y-3 text-sm">
      ...
      <p className="text-muted-foreground">
        Import complete. {result.avatar_ids.length} avatars have been
