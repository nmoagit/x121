/**
 * Shared hook for drag-and-drop avatar import flow.
 *
 * Encapsulates the import state, handlers, and bulk-create mutation
 * so that both ProjectAvatarsTab and ProjectGroupsTab can share
 * identical import behavior without duplicating code.
 *
 * Supports two modes:
 * - Name-only (legacy): creates avatars from name list
 * - Asset-aware: creates avatars + uploads images/videos from folders
 */
...
import {
  partitionAvatarFiles,
  type FileAssignments,
  type UnmatchedAvatarFiles,
} from "@/features/avatars/hooks/useAvatarImportBase";
...
/**
 * @param projectId - Primary project for creating new avatars.
 * @param allAvatars - Optional avatars from ALL displayed projects.
 *   When provided, existing-avatar lookups search this list instead of
...
export function useAvatarImport(projectId: number, allAvatars?: Avatar[], pipelineId?: number) {
  ...
  function handleFolderDrop(result: FolderDropResult) {
    ...
    // Flatten all grouped payloads for the modal's avatar list
  ...
  const handleImportConfirmWithAssets = useCallback(
    async (
      ...
      console.info(
        `[Import] Starting import of ${totalCount} avatar(s) for project ${projectId}`,
      );
      ...
      if (newNames.length > 0) {
        console.info(
          `[Import] Phase 1: Creating ${newNames.length} avatar(s)`,
        );
        setImportProgress({
          phase: "creating",
          current: 0,
        ...
        if (hasGroupAssignments) {
          ...
          try {
            ...
            { const msg = `Failed to create avatars: ${String(err)}`; errors.push(msg); addLogEntry(importLogEntry("error", msg, projectId)); }
            setImportProgress(null);
            setImportOpen(false);
      ...
      for (const payload of existingPayloads) {
        ...
        if (existing) {
          ...
          { const msg = `Could not find existing avatar "${payload.rawName}"`; errors.push(msg); addLogEntry(importLogEntry("error", msg, projectId)); }
        }
      }

      // Combine all payloads that have a resolved avatar ID
      ...
      console.info(
        `[Import] Phase 1 complete: ${createdAvatars.length} avatar(s) created`,
      );
      ...
      if (imageAssets.length > 0) {
        ...
        // Fetch existing variant_types per avatar to avoid duplicates
        ...
        for (const cid of uniqueCharIds) {
          try {
            ...
            // If fetch fails, proceed without skip-check for this avatar
        ...
        for (let i = 0; i < imageResults.length; i++) {
          ...
          if (r.status === "rejected") {
            ...
            errors.push(msg);
            addLogEntry(importLogEntry("error", msg, projectId, {
              file: imageAssets[i]!.asset.file.name, avatar: imageAssets[i]!.charName,
            }));
          }
        }
      ...
      if (metadataPayloads.length > 0) {
        console.info(
          `[Import] Phase 3: Uploading metadata for ${metadataPayloads.length} avatar(s)`,
        );
        setImportProgress({
        ...
        const metadataTasks = metadataPayloads.map((payload) => async () => {
          ...
          // Skip if avatar already has the specific source files being uploaded.
          ...
          // Store raw sources (matching AvatarMetadataTab pattern)
          ...
          addLogEntry(importLogEntry("info", `Metadata (${sources}) imported for ${payload.rawName}`, projectId, {
            avatar: payload.rawName, sources,
          }));
          return "uploaded" as const;
        });
        ...
        for (let i = 0; i < metadataResults.length; i++) {
          ...
          if (r.status === "rejected") {
            ...
            errors.push(msg);
            addLogEntry(importLogEntry("error", msg, projectId, {
              avatar: metadataPayloads[i]!.rawName,
            }));
          }
        }
      ...
      if (videoAssets.length > 0) {
        ...
        const videoTasks = videoAssets.map(({ charId, asset, charName }) => async () => {
          ...
          if (!sceneType) {
            ...
            errors.push(msg);
            addLogEntry(importLogEntry("error", msg, projectId, {
              file: asset.file.name, avatar: charName, sceneSlug: parsed.sceneSlug,
            }));
            return "skipped" as const;
          }
          ...
          const existingScenes = await api.get<Scene[]>(`/avatars/${charId}/scenes`);
          ...
          if (!scene) {
            const sceneId = await createSceneForAvatar(charId, sceneType.id, trackId, null);
        ...
        for (let i = 0; i < videoResults.length; i++) {
          ...
          if (r.status === "rejected") {
            ...
            errors.push(msg);
            addLogEntry(importLogEntry("error", msg, projectId, {
              file: videoAssets[i]!.asset.file.name, avatar: videoAssets[i]!.charName,
            }));
          }
        }
    ...
    [bulkCreate, avatars, sceneCatalogue, tracks, projectId, queryClient],
  );
