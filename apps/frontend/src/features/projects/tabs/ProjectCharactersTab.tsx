...
export function ProjectCharactersTab({ projectId }: ProjectCharactersTabProps) {
  ...
  function handleSaveEdit(characterId: number, data: UpdateCharacter) {
  ...
  return (
    ...
    >
      <Stack gap={4}>
        ...
        {/* Add character modal */}
        <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Add Character" size="sm">
          <Stack gap={4}>
            <Input
        ...
        <CharacterEditModal
          ...
          onDeleteRequest={(char) => setDeleteTarget(char)}
        />
