/**
 * Character-level workflow overrides panel.
 *
 * Shows scene_type x track workflow assignments inherited from the scene
 * catalogue, filtered to scenes enabled for this character.
 */
...
import { useCharacterSceneSettings } from "./hooks/use-character-scene-settings";

/* --------------------------------------------------------------------------
   Props
   -------------------------------------------------------------------------- */

interface CharacterWorkflowOverridesProps {
  characterId: number;
}
...
export function CharacterWorkflowOverrides({ characterId }: CharacterWorkflowOverridesProps) {
  const { data: allEntries, isLoading: loadingEntries } = useSceneCatalogue();
  const { data: settings, isLoading: loadingSettings } = useCharacterSceneSettings(characterId);
