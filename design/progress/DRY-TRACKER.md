...
| DRY-017 | Middleware/router construction duplicated in main.rs and tests/common/mod.rs | PRD-02, PRD-05, PRD-10, PRD-09 | `flagged` | `build_test_app()` duplicates the full middleware stack from `main.rs` (CORS, request ID, timeout, tracing, catch panic). PRD-09 worsened this: `build_app_with_orchestrator()` in `script_api.rs` was a 60-line copy of `build_test_app()`. Partially fixed: extracted `build_test_app_with()` to `common/mod.rs` with optional `ScriptOrchestrator` parameter, so all test files share one middleware builder. Remaining: main.rs still has its own copy. Extract `build_app_router(state: AppState, config: &ServerConfig) -> Router` to `trulience_api::router` module. |
...
| DRY-083 | `DataResponse<T>` adoption in existing handlers | PRD-06+ | `flagged` | `notification.rs` (7 instances) and `validation.rs` (7 instances) still use ad-hoc `serde_json::json!({ "data": ... })` instead of the shared `DataResponse<T>` from `api/src/response.rs`. Migrate when these handlers are next touched. |
