...
| DRY-017 | Middleware/router construction duplicated in main.rs and tests/common/mod.rs | PRD-02, PRD-05, PRD-10 | `flagged` | `build_test_app()` duplicates the full middleware stack from `main.rs` (CORS, request ID, timeout, tracing, catch panic). PRD-10 worsened this -- adding `event_bus` to AppState required parallel changes to both files. The test CORS layer is hardcoded instead of calling `build_cors_layer`. Extract `build_app_router(state: AppState, config: &ServerConfig) -> Router` to `trulience_api::router` module. |
