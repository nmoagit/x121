...
| PRD-132 | Queue Manager & Intelligent Job Allocation | 1 | `planning` | — | Full queue visibility (all job states, not just pending), ComfyUI-integrated job cancellation (interrupt GPU), job reassignment between workers, worker drain mode (finish current work before stopping), intelligent load-aware allocation (replace naive first-available), admin queue manipulation (move to front, hold, bulk cancel, redistribute), queue statistics dashboard. New held state, drain mode on instances, job-instance tracking. Deps: PRD-07, PRD-08, PRD-05, PRD-46. |
